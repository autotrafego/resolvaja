$$(document).on('page:init','.page[data-name="form-newservice"]', function(e){
        var page = e.detail;
        ...
        $("#tiposervice option:selected").each(function() {
         ...
         var tiposervice2 = $("#tiposervice option:selected").val();
         console.log(tiposervice2);
         //Set all links to google
        //  $$('#tiposervice').value('href', 'http://google.com');
         $$('#tiposervice').on('change',function (a) {
            console.log("mudou");
            var tiposervice = $(this).val();
        ...
        $$('#btnSalvar').on('click',function () {
                ...
                $$('tiposervice').val('New value here');
                firebase.database().ref().child('services').push(formData)
